...
rem Chek
echo.
if "%reset%"=="0" (set /p check="Do you want to restore the world [Y/N]?")
if "%reset%"=="0" (IF /i "%check%" neq "y" (exit))
echo.
...
if %errorlevel% equ 1 (color 04 & echo. There are no worlds with this name, try again. & echo %reset%> "%m_path%\Survival Backups\reset_wn.tmp" & timeout /t 2 /nobreak & call ".\Restore the world SM.bat" & exit)
echo.
